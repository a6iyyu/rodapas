<header class="w-full border border-b-slate-800/50 bg-white shadow-lg">
    <section class="container mx-auto w-9/10 flex items-center justify-between py-4">
        <a href="{{ route('beranda') }}" class="font-bold text-xl">
            RODAPAS
        </a>
        <a href="{{ route('bayar') }}" class="flex items-center gap-4 text-sm border border-slate-200 px-4 py-2.5 rounded transition-all duration-300 ease-in-out lg:hover:bg-slate-200">
            <i class="fa-solid fa-wallet"></i>
            <h5 class="">Bayar</h5>
        </a>
    </section>
</header>